...
User Function QSIntLoj(lAutomato,cAlias,cMarca,nOpc)
	...
	*/					   
	...
	PRIVATE cCadastro  := "Selção de Pedidos"	//"Exclusao dos Documento de Saida"
...
|Gera Array com as informaçães para executar a inclusão no Sigaloja|
|Desenvolvedo: Diogo Melo                                          |
...
User Function ImpSc5()
	...
	cSC5Qry += " WHERE D_E_L_E_T_ != '*' AND C5_TIPO = 'N' "
	cSC5Qry += " AND C5_BLQ = ' ' "
	cSC5Qry += " AND C5_FILIAL = '"+cFilant+"'" 
	cSC5Qry += " AND C5_OK in (' ','PA') " 
	cSC5Qry += " AND C5_P_AG != 1 "
	cSC5Qry += " AND C5_P_DTRAX != ' ' "
	cSC5Qry += " AND C5_NOTA = ' ' "
	cSC5Qry += " AND C5_ORIGEM != 'QSIntLoj' "
	cSC5Qry += " and C5_MKOK != ' ' "
	...
	While (cAliasSC5)->(!EOF())
		...
		If !Empty((cAliasSC5)->C5_MKOK) .AND. (cAliasSC5)->C5_OK $ "  |PA|"
			...
			nReg := u_RETSC6((cAliasSC5)->C5_NUM, cFiltro)
			nBon := u_BoniSC6((cAliasSC5)->C5_NUM, cFiltro)
			...
			If nReg > 0 
			cIdMember := POSICIONE("SA1", 1, xFilial("SA1") + (cAliasSC5)->C5_CLIENTE+(cAliasSC5)->C5_LOJACLI, "A1_P_DTRAX")
			...
			Iif(Empty((cAliasSC5)->C5_VEND1),"000001",cSC5->C5_VEND1),/*aCab[1][10]*/,;
			/*SL1->LQ_NUMMOV||aCab[1][11]*/'01',;
			(cAliasSC5)->C5_P_DTRAX, /*aCabDados[1][14]*/;
			cIdMember, /*aCabDados[1][15]*/;
			cMsgNota}) //aCabDados[1][16]
			Endif
	...
	For nX := 1 to Len(aCabDados)
		...
		While (cAliasSC6)->(!eof())
			...
			aB2Saldo := u_RetSaldo(Alltrim((cAliasSC6)->C6_PRODUTO), (cAliasSC6)->C6_LOCAL, (cAliasSC6)->C6_QTDVEN)
			
		If Len(aB2Saldo) > 0
				
			If (cAliasSC6)->C6_QTDENT < (cAliasSC6)->C6_QTDVEN .and. Empty((cAliasSC6)->C6_NOTA)
				
				IF aB2Saldo[1][2] >= (cAliasSC6)->C6_QTDVEN
			...
			Else
				Conout("O produto "+(cAliasSC6)->C6_PRODUTO+" Foi baixado total ou parcialmente no pedido "+(cAliasSC6)->C6_NUM)
			EndIf
...
Static Function SB2Saldo(cEmp, cCodigo, cLocal)
	Local nSaldo := 0
	...
	If Len(aCabDados) > 0
		...
		lMsErroAuto := .F.
			...
			If !lTemSL1
				...
				aAdd( _aCab, {"LQ_P_IDMEM" , aCabDados[nX][13] , NIL} )
				aAdd( _aCab, {"LQ_MENNOTA" , aCabDados[nX][14] , NIL} ) 
				IF cReserva == '1'
				...
				If Len(aItePed) > 0
						...
						For nZ:= 1 to Len(aItePed[nY])
							...
							If aCabDados[nX][1] == aItePed[nY][nZ][1] .and. aCabDados[nX][2] == aItePed[nY][nZ][2]
								...
								If cReserva == '1'
									...
									aAdd( _aItem[Len(_aItem)], {"LR_LOTECTL", aItePed[nY][nZ][12] , NIL} ) 
					...
					aAdd( _aParcelas[Len(_aParcelas)], {"L4_FORMA" , "04" , NIL} )
					aAdd( _aParcelas[Len(_aParcelas)], {"L4_ADMINIS" , " " , NIL} )
					aAdd( _aParcelas[Len(_aParcelas)], {"L4_NUMCART" , "000" , NIL} )
...
//--
					If lMsErroAuto
						cMsgErro := "/autocom/logs/"//MostraErro()
						//cArqErro := subs(time(),1,2)+subs(time(),4,2)+subs(time(),7,2)+".log"
				        cArqErro := aCabDados[nX][2]+'.log'
						cMsg2Erro:= MostraErro(cMsgErro, cArqErro ) 
						//Conout(cMsq2Erro)
						//DisarmTransaction()
					Else

					lTemSL1 := SL1->(MSSEEK(cFilant+SL1->L1_NUM))
					
					If lTemSL1
							
				    LjGrvLog("","Pedido encontrado na SL1",lTemSL1)

						If SC5->(MsSeek(aCabDados[nX][1] + aCabDados[nX][2])) //C5_FILIAL+C5_NUM
							Reclock("SC5",.F.)
							SC5->C5_OK     := "IN"
						    ...
						    Conout("Filial: "+aCabDados[nX][1]+"|Pedido: "+aCabDados[nX][2]+"| Não encontrado para atualizar integração.")
						EndIf
					EndIf
							Conout("Sucesso na execução do ExecAuto")
					EndIf
//--
				Endif
			Endif
