...
User Function ImpSc5(lAutomato)
	...
	Private lAutomato := IIF(Empty(lAutomato),lAutomato,.T.)
	...
	Else
		cSC5Qry += " AND C5_OK = ' '"
		cSC5Qry += " AND C5_P_AG = 0 " 
		cSC5Qry += " AND C5_EMISSAO >= '20190105' " 
	EndIf
	...
	For nX := 1 to Len(aCabDados)
		...
		While (cAliasSC6)->(!eof())
			...
			If (cAliasSC6)->C6_QTDENT < (cAliasSC6)->C6_QTDVEN .and. Empty((cAliasSC6)->C6_NOTA)
				...
				ElseIf nB2Saldo < (cAliasSC6)->C6_QTDVEN .and. nB2Saldo > 0
					...
					(cAliasSC6)->(dbSkip())
				Else
					Conout("Item "+(cAliasSC6)->C6_PRODUTO+" do pedido "+(cAliasSC6)->C6_NUM+" Nao integrado, saldo insuficiente.")
					(cAliasSC6)->(dbSkip())
				EndIf
	...
	Conout("******************************************************")
	Conout("Foram processados " +cValtochar(nCountSC5) + " Pedidos")
	Conout("Foram processados " +cValtochar(nCountSC6) + " Itens")
	Conout("******************************************************")
	...
	Else
		u_MyLOJA701( ,lAutomato,"2",aCabDados,aItePed)
		RpcClearEnv()  
	Endif
...
User Function MyLOJA701(oSay,lAutomato,cReserva,aCabDados,aItePed)
	...
	/*
	If lAutomato
	PREPARE ENVIRONMENT EMPRESA _cEmpresa FILIAL _cFilial MODULO "12"
	EndIf
	*/
	//Indica inclusão
	lMsHelpAuto := .T.
		...
		Else
			...
			For xI := 1 to Len(aCabDados)
				If SC5->(MsSeek(aCabDados[xI][1] + aCabDados[xI][2])) //C5_FILIAL+C5_NUM
					Reclock("SC5",.F.)
					SC5->C5_OK := "IN"
					SC5->C5_NOTA := 'Integrado' //Bloquear Pedido de venda para não
					SC5->C5_SERIE := 'INT'      //ser faturado pelo módulo 05
					MsUnlock()
				EndIf
			Next
			//---------------------------
			...
			Else
				Alert("Sucesso - Pedidos importados")
			Endif
		EndIf
		/*
		If lAutomato
