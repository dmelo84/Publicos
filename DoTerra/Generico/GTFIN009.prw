...
If nOpc == 1 //Preenche a partir da posição 111 até a 230
	...
	//	IF ALLTRIM(SM0->M0_CODIGO) == "QU"
		...
		Else
			...
			cRet := STRZERO(VAL(cCodre),6) + "01" + SUBSTR(SE2->E2_P_CGCON,1,14) + "17" + STRZERO(MONTH(SE2->E2_EMISSAO,2),2)+ STRZERO(YEAR(SE2->E2_EMISSAO,4),4)
    ...
    If cModPag $ "16"  // DARF
		...
		Else
			cCodre := SE2->E2_CODRET
			cRet := STRZERO(VAL(cCodre),6) + "01" + SUBSTR(SE2->E2_P_CGCON,1,14) + "16" + GRAVADATA(SE2->E2_EMISSAO,.F.,5)
    ...
    If cModPag $ "22/23"  // GARE
			...
			Else
				cCodre := SE2->E2_CODRET
				cRet := cCodre + SPACE(6-LEN(cCodre)) + "01" + SUBSTR(SE2->E2_P_CGCON,1,14) + "22" + GRAVADATA(SE2->E2_VENCREA,.F.,5)
				cRet += STRZERO(VAL(SUBSTR(SE2->E2_P_INSCR,1,12)),12) + REPL("0",13) + STRZERO(MONTH(SE2->E2_EMISSAO,2),2)+ STRZERO(YEAR(SE2->E2_EMISSAO,4),4)
