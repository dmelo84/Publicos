			...
			For nI := 1 to LEN(oXmlItm)
				...
				For nIa := 1 to XmlChildCount(oXmlItm[nI])
					...
					If UPPER(Alltrim(XmlGetChild(oXmlItm[nI], nIa):REALNAME)) == "CODESPEC"
						IF Alltrim(XmlGetChild(oXmlItm[nI], nIa):TEXT) == ALLTRIM(GetMV("ES_PRDMOT", .F., "23000004")) 
							lPrdMot	:= .T.
						ENDIF 
					ENDIF 
				...
				lDPCServ := SuperGetMv( "AL_DPCSERV", .F., .T.)	
				If lDPCServ 
				...
				------------------------------------------------------------------------------------------*/
				cNovoProd	:= SUPERGETMV("AL_PROPFAT",.F.,"", cFil)
				IF !EMPTY(cNovoProd)
					cSerProd	:= cNovoProd
				ENDIF
				...
				------------------------------------------*/
				ConOut("****Filial PV**** " + cFil)
//					IF ALLTRIM(cFil) $ cFilCdb
				
				ConOut("Realiza tratativa Motoboy inicio " + dtoc(DDATABASE) + " - " +  TIME())
				IF GetMV("ES_HABMOT",.F., .T.)//|Habilita tratativa Motoboy|  
					ConOut("Tratativa Motoboy Habilitada")
					...
					IF !lPrdMot
						IF SZK->(DBSEEK(SM0->M0_CODIGO + cFil ))
						 	IF SZK->ZK_XMOTOB == "S" .AND. nValIt == GetMV("ES_VALMOT",.F., 12) 
						 		lPrdMot := .T.
						 	ENDIF 
						ENDIF
					ENDIF  
					...
					IF lPrdMot
						cTes := GetMV("ES_TESMOT",.F.,"509") 
						cSerProd := GetMV("ES_PRDMOT", .F., "23000004") 
						aAdd(aPedido, {"C5_XMOTOB", "1", NIL})//|Pedido de Motoboy|
						
						ConOut("Tratativa Motoboy -  TES "+ cTes + " Produto: " + cSerProd) 
					ENDIF 
...
User Function FSAtuCli(aCliente, aPedido, cMsg, lCli, cTipo, aPagam)
	...
	If lContinua
		aCliente[aScan(aCliente, {|X| X[1] == "A1_NOME"})][2] := NoAcento(aCliente[aScan(aCliente, {|X| X[1] == "A1_NOME"})][2])
	Endif
...
User Function FSGeraParc(cFil, cPedido, aParc)
	...
	For nP := 1 to len(aParc)
		...
		cIdTran  := aParc[nP][aScan(aParc[nP], {|X| X[1] == "IDTRANSACAO"})][2]

		If lMotoboy .AND. cTipo == "CC"
...
User Function FSPLEFAT(cXml,cIdPleres)
	...
	If cDesRet=="X" //Xclinic
		...
		ConOut("*********************************************************")
		ConOut("* FSLIBPLE - Filial: " + cFilAnt + " - ID: " + cIdPleres+" - " + DtoC(Date()) + " - " + Time() + " Integrando XClinic!")
		ConOut("*********************************************************")
	...
	// Cria a instância da classe client
		...
		ConOut("*********************************************************")
		ConOut("* FSLIBPLE - Filial: + " + cFilAnt + "Parametro ES_XCLWFAT - Link: " + oWSDL:_URL)
		ConOut("*********************************************************")
	...
	ElseIf cDesRet=="P" //Pleres
		...
		ConOut("*********************************************************")
		ConOut("* FSLIBPLE - Filial: " + cFilAnt + " - ID: " + cIdPleres+" - " + DtoC(Date()) + " - " + Time() + " Integrando Pleres!")
		ConOut("*********************************************************")
	...
	// Cria a instância da classe client
		...
		ConOut("*********************************************************")
		ConOut("* FSLIBPLE - Filial: + " + cFilAnt + "Parametro ES_PLEWFAT - Link: " + oWSDL:_URL)
		ConOut("*********************************************************")
	...
	ElseIf cDesRet=="C" //Clinux
		...
		ConOut("*********************************************************")
		ConOut("* FSLIBPLE - Filial: " + cFilAnt + " - ID: " + cIdPleres+" - " + DtoC(Date()) + " - " + Time() + " Integrando Clinux!")
		ConOut("*********************************************************")
	...
	// Cria a instância da classe client
		...
		ConOut("*********************************************************")
		ConOut("* FSLIBPLE - Filial: + " + cFilAnt + "Parametro ES_CLIWFAT - Link: " + oWSDL:_URL)
		ConOut("*********************************************************")
