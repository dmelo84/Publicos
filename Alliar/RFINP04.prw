...
/*/{Protheus.doc} RFINP04
RelatÛrio de fluxo de caixa por natureza.
...
Static Function ReportDef()
	...
	IF MV_PAR08 == 2 //| RelatÛrio di·rios
		...
		nDias := DateDiffDay(mv_par05,mv_par06)
	 	...
	 	While  nDias > 30 .OR. mv_par05 > mv_par06
	  		...
	  		If (mv_par05 > mv_par06)
	  			...
	  			Aviso("RFINP04","O parametro data de, deve ser menor que o data ate: ",{"OK"},1)

			ElseIf nDias > 30

				Aviso("RFINP04","SÛ È permitido perÌodo de atÈ 31 dias",{"OK"},1)
	...
	oReport := TReport():New("RFINP04","FLUXO DE CAIXA POR NATUREZA","",{|oReport| PrintReport(oReport)},"IMPRESSAO DO FLUXO DE CAIXA POR NATUREZA")
	oSection1 := TRSection():New(oReport,OemToAnsi("MOVIMENTA«’ES"),{"FCAIXA"})


	If mv_par09 == 1 //AnalÌtico
		...
		TRCell():New(oSection1,"BANCO"		 	,"FCAIXA"	,"BANCO"		,"@!",03)
		...
		TRCell():New(oSection1,"PRODUTO"		,"FCAIXA"	,"PRODUTO"		,"@!",40)//codigo+descVixe produto
...
FUNCAO RESPONS¡VEL PELA IMPRESS√O DO RELAT”RIO
@author Fabio Sales | www.compila.com.br
@since 12/10/2016
@version 1
@see (www.compila.com.br)
...
Static Function PrintReport(oReport)
	...
	If MV_PAR09 == 1 // AnalÌtico
		...
		While  FCAIXA->(!Eof() )
			...
			//⁄ƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒ8ø
			//≥N„o imprime as linhas com baixas canceladas. Executado para o analÌtico≥
			//¿ƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒ8Ÿ
			/*
			If FCAIXA->LOTE <> ''
...
/*/{Protheus.doc} PrintReport
SELE«√O DOS DADOOS  PARA IMPRESS√O
@author Fabio Sales | www.compila.com.br
@since 12/10/2016
@version 1
@see (www.compila.com.br)
...
Static Function fSelDados()
	...
	_cquery += " 	AND D_E_L_E_T_ = ' 'AND A6_BLOCKED='2'AND A6_FLUXCAI='S' "
	If Select("CTAS") > 0
		CTAS->(dbCloseArea())
	EndIf
	...
	If Mv_PAR09 == 1 //AnalÌtico
		...
		//Movimentos
		//***************
		clQuery += BaseData(_cContas) &&Edson Melo - AtenÁ„o ao realizar alteraÁ„o nessa funÁ„o, pois reflete em todos os relatÛrios (AnalÌtico, SintÈtico Mensal e SintÈtico Di·rio)

	Else //SintÈtico


		IF MV_PAR08 == 1  // SintÈtico > Mensal

			//***************
			//Movimentos
			//***************

			clQuery := " SELECT FILIAL, TIPO, NATUREZA, DESCRI, PRODUTO, SALDO_ANT, SUM(VALPER) * CASE TIPO WHEN 'DESPESAS' THEN -1 ELSE 1 END AS VALPER "+CRLF
			clQuery += " FROM ( "+CRLF
			...
			clQuery += "               DESCRI, " +CRLF
			clQuery += "               PRODUTO, " +CRLF
			clQuery += "               0 SALDO_ANT, "+CRLF
			clQuery += "               SUM(VALPER) AS VALPER

			clQuery += "       FROM ( " + CRLF
			clQuery +=                BaseData(_cContas) + CRLF &&Edson Melo - AtenÁ„o ao realizar alteraÁ„o nessa funÁ„o, pois reflete em todos os relatÛrios (AnalÌtico, SintÈtico Mensal e SintÈtico Di·rio)
			clQuery += "       ) AS T2 " + CRLF
			clQuery += "       GROUP BY FILIAL, ORIEN, NATUREZA, PRODUTO, DESCRI " + CRLF

			clQuery += " ) AS T GROUP BY FILIAL, TIPO, NATUREZA, DESCRI, PRODUTO, SALDO_ANT " +CRLF
			clQuery += " ORDER BY TIPO, NATUREZA, FILIAL "+CRLF

		ELSE  //Sint·tico > Di·rio
			...
			clQuery +=                BaseData(_cContas) + CRLF &&Edson Melo - AtenÁ„o ao realizar alteraÁ„o nessa funÁ„o, pois reflete em todos os relatÛrios (AnalÌtico, SintÈtico Mensal e SintÈtico Di·rio)
			clQuery += "       ) AS T2 " + CRLF
			clQuery += "       GROUP BY FILIAL, ORIEN, NATUREZA, DESCRI, PRODUTO, DATA, COND " + CRLF

			clQuery += " )  AS T  "+CRLF
			clQuery += " GROUP BY TIPO, NATUREZA, FILIAL, DESCRI, PRODUTO "+CRLF
			clQuery += " ORDER BY TIPO, NATUREZA, FILIAL, DESCRI, PRODUTO "+CRLF
		EndIf
	EndIf
	...
	MemoWrite(GetTempPath(.T.) + "RFINP04.SQL", clQuery)
...
Static Function ValPerg(cPerg)
	...
	PutSx1(cPerg,"09","Tipo Rel.","","","mv_ch9","N",01,0,0,"C","","","","","mv_par09","AnalÌtico","","","","SintÈtico","","","","","","","","","","","",{"Detalhado ou resumido? "},{},{},"" )
...
±±∫Programa  ≥RFINP04   ∫Autor  ≥Microsiga           ∫ Data ≥  12/29/16   ∫±±
...
Local cBaseQry := ""
Local lSpbInUse := SpbInUse()
Local cTabela14 := FR470Tab14()//StaticCall(FINR470,FR470Tab14)
cBaseQry += " SELECT FILIAL, DATA, LOTE, XIDFLG, BANCO, PREFIXO, NUMERO, PARCELA, TIPO, CLIFOR, LOJA, BENEF, SEQ, ORIEN, RECPAG, NATUREZA, PRODUTO, COND, DESCRI, VALPER, RECE5 "+CRLF
cBaseQry += " FROM ("+CRLF
...
cBaseQry += " 		ISNULL(RTRIM(D1_COD)+' - '+B1_DESC,'') PRODUTO, " +CRLF
...
cBaseQry += " LEFT JOIN " + RetSqlName("SD1") + " SD1 ON "+CRLF
cBaseQry += "  D1_DOC = E5_NUMERO AND E5_FILIAL = D1_FILIAL AND E5_PREFIXO = D1_SERIE AND D1_ITEM = '0001' AND E5_FORNECE = D1_FORNECE AND E5_LOJA = D1_LOJA  AND D1_CF <> '' "+CRLF
cBaseQry += "  AND SD1.D_E_L_E_T_ = '' "+CRLF

cBaseQry += " LEFT JOIN " + RetSqlName("SB1") + " SB1 ON "+CRLF
cBaseQry += "  B1_COD = D1_COD  "+CRLF
cBaseQry += "  AND  SB1.D_E_L_E_T_ = '' "+CRLF
...
cBaseQry += " GROUP BY FILIAL, DATA, LOTE, XIDFLG, BANCO, PREFIXO, NUMERO, PARCELA, TIPO, CLIFOR, LOJA, BENEF, SEQ, ORIEN, RECPAG, NATUREZA, PRODUTO, COND, DESCRI, VALPER, RECE5 "+CRLF
