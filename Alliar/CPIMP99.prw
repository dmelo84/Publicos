...
//� Importa XML �
//���������������
Processa({||  U_CPIMP99A(aArqFullPath) })		
...
@see (links_or_references)
/*/
User Function CPIMP99A(aPathArq)
Local cMsgErro	:= ""
Local nTotArq, nI
...
@see (links_or_references)
/*/
User Function CPIMP99B(cPathPai, cPathFilho)
Local cMsgErro	:= ""
Local nTotArq, nI
	...
	IF !EMPTY(cPathFull) .AND. !EMPTY(cNomeArq)
		...
		IF  (nHdlArq	:= FT_FUSE(cFullTemp) ) >= 0				
			...
			for nI:= 1 to Len(aCabecArq)
				...
				ELSE
					IF cAliasImp == "SE2"
						AADD(aCabecX3, {alltrim(aCabecArq[nI]), "C", 0})
					ELSE
						AADD(aCabecX3, {alltrim(SX3->X3_CAMPO), "", 0})
					ENDIF										
				ENDIF
			next nI
			...
			WHILE !FT_FEOF()
				nReg++					
				IncProc("Importando registros... "+STRZERO(nReg,6)+" de "+STRZERO(nTotLin,6))					
				...
				IF nCabecArq == nQtdeCol
					...
					//aRetAux	:= U_CPIMP99X(cAliasImp,aDados,.F.,aItens) 	//inclusao
					aRetAux	:= U_CPIMP99X(cAliasImp,aDados,.T.,aItens)		//inclusao e alteracao
					//aRetAux		:= {.t.,""}			
					IF aRetAux[1]
						/*
						IF cAliasImp == "SN1"
...
ELSE
	...
	for nI := 1 to Len(aPastas)
		...
		IF nAux <> 0
			CONOUT("### CPIMP99.PRW [DirTemp] | Nao foi possivel criar o diretorio ["+alltrim(aPastas[nI])+"]. Cod. Erro: "+alltrim(str(FError())) )
		ENDIF		
...
IF !EMPTY(cPathFull)
	...
	IF !EMPTY(cPathFull) .AND. !EMPTY(cNomeArq)
		...
		IF  (nHdlArq	:= FT_FUSE(cFullTemp) ) >= 0				
			...
			for nI:= 1 to Len(aCabecArq)
				IF SX3->(DBSEEK(ALLTRIM(UPPER(aCabecArq[nI]))))
					AADD(aCabecX3, {alltrim(SX3->X3_CAMPO), SX3->X3_TIPO})
				ELSE
					AADD(aCabecX3, {alltrim(SX3->X3_CAMPO), "" })										
				ENDIF
			next nI
