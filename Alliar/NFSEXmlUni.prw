#include "protheus.ch"
#include "tbiconn.ch"
//#include "fwlibversion.ch"  //HFP 20210209
...
User function nfseXMLUni( cCodMun, cTipo, dDtEmiss, cSerie, cNota, cClieFor, cLoja, cMotCancela, aAIDF )
	...
	Local cNatBusc   := ""
	Local cMsgSX5	:= ""
	Local cLibVersion := __FWLibVersion()  //HFP 20210209 allTrim( FwLibVersion() )
	...
	If cTipo == "1" .And. Empty(cMotCancela)
		...
		If DbSeek(xFilial("SF2")+cNota+cSerie+cClieFor+cLoja)
			...
			If !SF2->F2_TIPO $ "DB"
				...
				aadd(aDest,AllTrim(SA1->A1_CGC))  //01
				...
				Else
					...
					aadd(aDest,"EXTERIOR")
				EndIf
				aadd(aDest,Upper(SA1->A1_EST))  //09
				...
				aadd(aDest,Iif(!Empty(SA1->A1_SIMPNAC),SA1->A1_SIMPNAC,"2"))  //20
				aadd(aDest,Iif(SA1->(FieldPos("A1_INCULT"))> 0 , Iif(!Empty(SA1->A1_INCULT),SA1->A1_INCULT,"2"), "2"))  //21
				aadd(aDest,SA1->A1_TPESSOA)  //22
				aadd(aDest,SF2->F2_DOC)  //23
				aadd(aDest,SF2->F2_SERIE)  //24
																...
																If(!Empty(cNumMatOb),aadd(aConstr,(cNumMatOb)),aadd(aConstr,"") )	// Numero de Mtricula para Obra
																	If(!Empty(cNumEncap),aadd(aConstr,(cNumEncap)),aadd(aConstr,"") )	// NumeroEncapsulamento
																		...
																		If !Empty(aNota)
																			...
																			//  alliar X jacarei
																			//
																			// prepara variavel para pegar codigo tomador e usar nos tratamentos.
																			// inicialmente somente para jacarei.
																			//
																			cXCodTom := aDest[07] // SA1->A1_COD_MUN
																			If Len(cXCodTom) <= 5 .And. !(cXCodTom$'99999')
																				cXCodTom := UfIBGEUni(aDest[09]) + cXCodTom
																			EndIf
																			cXCPFCNPJ:= aDest[01]
																			// end alliar
																			...
																			// ALLIAR cString += ident( aNota, aProd, aTotal, aDest, aISSQN, aAIDF, dDateCom, cNatOp, cMunPrest )
																			cString += ident( aNota, aProd, aTotal, aDest, aISSQN, aAIDF, dDateCom, cNatOp, cMunPrest, cXCodTom, cXCPFCNPJ)
																			...
																			// ALLIAR cString += servicos( aProd, aISSQN, aRetido, cNatOper, lNFeDesc, cDiscrNFSe, aCST, aDest[22], SM0->M0_CODMUN, cF4Agreg ,nDescon )
																			cString += servicos( aProd, aISSQN, aRetido, cNatOper, lNFeDesc, cDiscrNFSe, aCST, aDest[22], SM0->M0_CODMUN, cF4Agreg ,nDescon, cXCodTom, cXCPFCNPJ)

																			// ALLIAR cString += valores( aISSQN, aRetido, aTotal, aDest, SM0->M0_CODMUN, aDeducao )
																			cString += valores( aISSQN, aRetido, aTotal, aDest, SM0->M0_CODMUN, aDeducao, cXCodTom, cXCPFCNPJ)
																			...
																			cString := AjstAlliar(cString)

																			//erro++  //provisorio para gerar erro antes de enviar
...
/*/{Protheus.doc} AjstAlliar
	Função para ralizar ajustes no XML antes do seu envio
	@type  Static Function
	@author Julio Teixeira - Compila
	@since 11/08/2020
	@version version
	@param cString - String com o XML da nota
	@return cString - String com XML ajustado conforme regras internas
/*/
Static Function AjstAlliar(cString)
	Local cFilReg := GETMV("MV_XSERFIL",.F.)
	Default cString := ""
		If !Empty(cString)
			//Ajustado para task https://compilabr.teamwork.com/#/tasks/19424113?c=6739867
			//Objetivo é evitar rejeição por envio da tag <cresponsavelretencao> quando <issretido> é igual a 2
			//If "<issretido>2</issretido>" $ cString .AND. cFilAnt $ cFilReg
			If "<issretido>2</issretido>" $ cString .AND. cFilReg
				cString := StrTran(cString,"<cresponsavelretencao>1</cresponsavelretencao>","")
			Endif
		Endif
Return cString
...
/*/
//-----------------------------------------------------------------------
Static Function ident( aNota, aProd, aTotal, aDest, aISSQN, aAIDF, dDateCom, cNatOp, cMunPrest,  cXCodTom, cXCPFCNPJ)
	...
	IF SM0->M0_CODMUN <> "3144805" //| ALLIAR -> NOVA LIMA 3144805 - TASK https://compilabr.teamwork.com/#/tasks/22653242
	...
	/* 1
		hfp-compila 20210302
		//ajustado a tag coforme task 21290704
		//comforme documentacao prefeitura jacarei. 
		cXCodTom, cXCPFCNPJ
	*/
	IF ALLTRIM(SM0->M0_CODMUN) == "3524402"  //Filial Jacarei

		IF cXCodTom == '3524402' .AND. Len(cXCPFCNPJ) == 14  // tomador tb jacarei e Juridica
			//Conforme reuniao juliane, fernando e vaney
			cString += "<issret>1</issret>"
		ELSE
			cString += "<issret>2</issret>"
		ENDIF
	...
	/*
	   hfp-compila  20210208
		ESPECIFICO PARA PREFEITURA SALVADOR conforme documentacao solicitando a mudanca
	*/
	IF ALLTRIM(SM0->M0_CODMUN) == "2927408"   //salvador

		cTmp:= aProd[1][02]  //codigo produto
		cTmp:= posicione( "SB1", 1, xFilial( "SB1" ) + cTmp, "B1_CODISS" )
		//-- Codigo do Item da Lista de Servico
		cString += "<itemlistaserv>"+alltrim(cTmp)+"</itemlistaserv>"
		//-- Codigo da Atividade CNAE
		cString += "<cnae>" + allTrim( aProd[1][19] ) + "</cnae>"
		cTmp:= aProd[1][02]  //codigo produto
		cTmp:= posicione( "SBZ", 1, xFilial( "SBZ" ) + cTmp, "BZ_CODISS" )
		//-- Codigo de Tributacao do Municipio
		cString += "<ctributmun>" + ALLTRIM(cTmp)+"</ctributmun>"
	ELSE
...
Static Function prest( cMunPSIAFI )
	...
	if !empty( allTrim( aTemp[4] ) )
		//| ALLIAR \/
		nTamComp	:= GETMV("AL_TAMCOMP",.F.,0)
		IF nTamComp > 0
			cString	+= "<compleend>"+LEFT(allTrim(aTemp[4]) ,nTamComp)+"</compleend>"
		ELSE
	...
	elseif !Empty(SM0->M0_COMPCOB)
		...
		nTamComp	:= GETMV("AL_TAMCOMP",.F.,0)
		IF nTamComp > 0
			cString	+= "<compleend>"+LEFT(allTrim(SM0->M0_COMPCOB) ,nTamComp)+"</compleend>"
		ELSE
...
Static Function prestacao( cMunPrest, cDescMunP, aDest, cMunPSIAFI )
	...
	If !Empty( allTrim( aDest[5] ) )
		...
		nTamComp	:= GETMV("AL_TAMCOMP",.F.,0)
		IF nTamComp > 0
			cString	+= "<complend>"+LEFT(allTrim(aDest[5]) ,nTamComp)+"</complend>"
		ELSE
...
Static Function tomador( aDest )
	...
	If lTomador
		...
		//| \/ ALLIAR \/ | ESPECIFICO PARA PREFEITURA JUIZ DE FORA
		IF ALLTRIM(SM0->M0_CODMUN) == "3136702"
			if allTrim( aDest[9] ) == "MG" .And. allTrim( aDest[7] ) == "36702"
				cString	+= "<inmuntom>" + allTrim( aDest[17] ) + "</inmuntom>"
				cString	+= "<inscmun>" + allTrim( aDest[17] ) + "</inscmun>"
				//if aDest[17] <> "ISENTO" .And. allTrim( aDest[17] ) == "3136702"
				//cString	+= "<inmuntom>" + allTrim( aDest[17] ) + "</inmuntom>"
				//else
				//cString	+=  "<inmuntom></inmuntom>"
				//endif
			endif
		ELSEIF ALLTRIM(SM0->M0_CODMUN) == "2927408" .OR.;
				ALLTRIM(SM0->M0_CODMUN) == "3144805" 
			//| ALLLIAR ***  SOMENTE PREENCHER MUNICIPIO TOMADOR QUANDO FOR DO MUNICIPIO ***
			//| 2927408 - BA-SALVADOR
			//| 3144805 - MG-NOVA LIMA - https://compilabr.teamwork.com/#/tasks/22653242
			
			//if allTrim( aDest[9] ) == "BA" .And. allTrim( aDest[7] ) == "27408"			
			if allTrim( aDest[9] ) == ALLTRIM(SM0->M0_ESTCOB) .And. allTrim( aDest[7] ) == RIGHT(ALLTRIM(SM0->M0_CODMUN),5)
				cString	+= "<inscmun>" + allTrim( aDest[17] ) + "</inscmun>"
			ENDIF
		ELSE
			If aDest[17] <> "ISENTO" .And. !Empty( aDest[17] )
		...
		//| ALLIAR \/
		IF SM0->M0_CODMUN == "3526902" //| Limeira |
			cString += "<atualizatomador>2</atualizatomador>"
		endif
		//| ALLIAR /\
...
/*/
//-----------------------------------------------------------------------
Static Function servicos( aProd, aISSQN, aRetido, cNatOper, lNFeDesc, cDiscrNFSe,aCST, cTpPessoa, cCodMun, cF4Agreg, nDescon, cXCodTom, cXCPFCNPJ)
	...
	For nX := 1 To Len( aProd )
		...
		If ( SC6->(FieldPos("C6_DESCRI")) > 0 .And. Len(aProd[nX]) > 40 .And. !Empty(aProd[nX][41]) ) .And. (!lNFeDesc .And. !GetNewPar("MV_NFESERV","1") == "1" .And. !Empty(GetMV("MV_CMPUSR")) )
			cString	+= "<discr>" + noacento(AllTrim(aProd[nX][41])+ cCargaTrb) + "</discr>"
		ElseIf !lNFeDesc
			cString	+= "<discr>" + noacento(AllTrim(cNatOper)+ cCargaTrb) + "</discr>"
		Else
			cString	+= "<discr>" + noacento(AllTrim(cDiscrNFSe)+ cCargaTrb )+ "</discr>"
		EndIf
		...
		/* 1
			hfp-Compila 20210302
		   - modificaco para atender prefeitura jacarei conforme taSk 21290704
		*/
		IF ALLTRIM(SM0->M0_CODMUN) == "3524402"  //jacarei

			IF cXCodTom == '3524402' .AND. Len(cXCPFCNPJ) == 14  // tomador tb jacarei e Juridica
				//Conforme reuniao juliane, fernando e vaney
				cString += "<issretido>1</issretido>"
			ELSE
		...
		/* 1
			hfp - Compila   20210301   TASK 21290704
			Incluido validacao separada para a filial jacarei, para nao afetar a logica utilizada abaixo
			e ja esta em producao, pois a situacao atual, somente para essa filial é nao enviar a tag
			ValIssRet, e da forma abaixo, mesmo parametro como.F. ele envia as 2 tags
		*/
		IF ALLTRIM(SM0->M0_CODMUN) == "3524402"


			IF cXCodTom == '3524402' .AND. Len(cXCPFCNPJ) == 14  // tomador tb jacarei e Juridica
				//Conforme reuniao juliane, fernando e vaney
				cString += "<valiss>0</valiss>"
				IF IIF(VALTYPE(aissret) == "A", LEN(aissret) >= 4, .F.)
					IF IIF(VALTYPE(aissret[4]) == "A", LEN(aissret[4]) >= nx, .F.)
						cString += "<valissret>" + allTrim( ConvType( aissret[4][nx]  ,15,2 ) ) + "</valissret>"
					ELSE	
						cString += "<valissret>0</valissret>"
					endif
				ELSE
					cString += "<valissret>0</valissret>"
				ENDIF
		...
		ELSE
			...
			If !( aIssRet[2] == 1 .AND. SuperGetMV("AL_ISSNET",.F.,.F.))	//JULIO TEIXEIRA ADICIONADO POR SOLICITAÇÃO NA TASK https://compilabr.teamwork.com/#/tasks/19847418
				cString += "<valiss>"    + allTrim( ConvType( aISSQN[nX][3]  ,15,2 ) ) + "</valiss>"
			Endif
			If !( aIssRet[2] == 2 .AND. SuperGetMV("AL_ISSNET",.F.,.F.))	//JULIO TEIXEIRA ADICIONADO POR SOLICITAÇÃO NA TASK https://compilabr.teamwork.com/#/tasks/19847418
...
//		cString += "<tributavel></tributavel>" //Nao Obrigat. - sem uso, existe no grupo valores
		...
		/*
		//| \/ ALLIAR \/ | ESPECIFICO PARA PREFEITURA SALVADOR    //hfp-Compila 20210208
		IF ALLTRIM(SM0->M0_CODMUN) == "2927408"   //27408
			cTmp:= aProd[Nx][02]  //codigo produto
			cTmp:= posicione( "SBZ", 1, xFilial( "SBZ" ) + cTmp, "BZ_CODISS" )
			cString += "<CodigoTributacaoMunicipio>"+ALLTRIM(cTmp)+"</CodigoTributacaoMunicipio>"
		ENDIF
		*/
...
/*/
//-----------------------------------------------------------------------
static function valores( aISSQN, aRetido, aTotal, aDest, cCodMun , aDeducao,  cXCodTom, cXCPFCNPJ)
...
Static Function faturas( aDupl )
	...
	If Len( aDupl ) > 0
		...
		For nX := 1 To Len( aDupl )
			...
			//| ALLIAR \/
			cDescFat	:= GETMV("AL_DESCFAT",.F.,"")
			cString += "<descFatura>"+ALLTRIM(cDescFat)+"</descFatura>"
			//| ALLIAR /\
