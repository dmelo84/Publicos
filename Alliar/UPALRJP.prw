...
/*/
//--------------------------------------------------------------------
User Function UPALRJP( cEmpAmb, cFilAmb )
...
If ( lOpen := MyOpenSm0(.T.) )
	...
	If lOpen
		...
		For nI := 1 To Len( aRecnoSM0 )
			...
			If SX3->( dbSeek( PadR( 'C7_PRODUTO', Len( SX3->(FIELDGET(FIELDPOS("X3_CAMPO"))) )) ) )
				//ESTA FUNÇÃO APENAS ATUALIZA COM 'S' A PROPRIEDADE TRIGGER DOS CAMPOS ADZ_QTDVEN E ADZ_PRODUT
				RecLock( "SX3", .F. )
				FieldPut( FieldPos( 'X3_TRIGGER' ), 'S' ) 
				dbCommit()
				MsUnLock()
			EndIf
			...
			If SX3->( dbSeek( PadR( 'CNB_PRODUT', Len( SX3->(FIELDGET(FIELDPOS("X3_CAMPO"))) )) ) )
				//ESTA FUNÇÃO APENAS ATUALIZA COM 'S' A PROPRIEDADE TRIGGER DOS CAMPOS ADZ_QTDVEN E ADZ_PRODUT
				RecLock( "SX3", .F. )
				FieldPut( FieldPos( 'X3_TRIGGER' ), 'S' ) 
				dbCommit()
...
aAdd( aSX3, { ;
	...
	{ 4																		, .T. }, ; //X3_TAMANHO
	{ 0																		, .T. }, ; //X3_DECIMAL
	{ 'Grupo Estoque'														, .T. }, ; //X3_TITULO
	{ 'Grupo Estoque'														, .T. }, ; //X3_TITSPA
	{ 'Grupo Estoque'														, .T. }, ; //X3_TITENG
	{ 'Grupo Estoque'											, .T. }, ; //X3_DESCRIC
	{ 'Grupo Estoque'											, .T. }, ; //X3_DESCSPA
	{ 'Grupo Estoque'											, .T. }, ; //X3_DESCENG
	{ '@!'																	, .T. }, ; //X3_PICTURE
	{ 'Vazio() .Or. A010Grupo()'																	, .T. }, ; //X3_VALID
...
//
// Campos Tabela SC7 referente a pedido rejeitado
//
aAdd( aSX3, { ;
	{ 'SC1'																	, .T. }, ; //X3_ARQUIVO
	{ ''																	, .T. }, ; //X3_ORDEM
	{ 'C1_XAPROV'															, .T. }, ; //X3_CAMPO
	{ 'C'																	, .T. }, ; //X3_TIPO
	{ 6																		, .T. }, ; //X3_TAMANHO
	{ 0																		, .T. }, ; //X3_DECIMAL
	{ 'Grp.Aprovacao'														, .T. }, ; //X3_TITULO
	{ 'Grp.Aprovacao'														, .T. }, ; //X3_TITSPA
	{ 'Grp.Aprovacao'														, .T. }, ; //X3_TITENG
	{ 'Grp.Aprovacao'											, .T. }, ; //X3_DESCRIC
	{ 'Grp.Aprovacao'											, .T. }, ; //X3_DESCSPA
	{ 'Grp.Aprovacao'											, .T. }, ; //X3_DESCENG
	{ '@!'																	, .T. }, ; //X3_PICTURE
	{ ''																	, .T. }, ; //X3_VALID
	{ Chr(128) + Chr(128) + Chr(128) + Chr(128) + Chr(128) + ;
	Chr(128) + Chr(128) + Chr(128) + Chr(128) + Chr(128) + ;
...
aAdd( aSX3, { ;
	...
	{ 10																		, .T. }, ; //X3_TAMANHO
	{ 0																		, .T. }, ; //X3_DECIMAL
	{ 'Justificativa'														, .T. }, ; //X3_TITULO
	{ 'Justificativa'														, .T. }, ; //X3_TITSPA
	{ 'Justificativa'														, .T. }, ; //X3_TITENG
	...
	{ ''																	, .T. }, ; //X3_CBOXENG
	{ ''																	, .T. }, ; //X3_PICTVAR
	{ 'Iif ( IsInCallStack("U_REJEITAR") , .T., .F.) '																	, .T. }, ; //X3_WHEN
	{ ''																	, .T. }, ; //X3_INIBRW
	{ ''																	, .T. }, ; //X3_GRPSXG
	{ ''																	, .T. }, ; //X3_FOLDER
	{ ''																	, .T. }, ; //X3_CONDSQL
...
aAdd( aSX3, { ;
	...
	{ 1																		, .T. }, ; //X3_TAMANHO
	{ 0																		, .T. }, ; //X3_DECIMAL
	{ 'Rejeitado'														, .T. }, ; //X3_TITULO
	{ 'Rejeitado'														, .T. }, ; //X3_TITSPA
	{ 'Rejeitado'														, .T. }, ; //X3_TITENG
	{ 'Rejeitado'											, .T. }, ; //X3_DESCRIC
	{ 'Rejeitado'											, .T. }, ; //X3_DESCSPA
	{ 'Rejeitado'											, .T. }, ; //X3_DESCENG
	{ '@!'																	, .T. }, ; //X3_PICTURE
	{ ''																	, .T. }, ; //X3_VALID
	{ Chr(128) + Chr(128) + Chr(128) + Chr(128) + Chr(128) + ;
	Chr(128) + Chr(128) + Chr(128) + Chr(128) + Chr(128) + ;
...
aAdd( aSX3, { ;
	...
	{ 1																		, .T. }, ; //X3_TAMANHO
	{ 0																		, .T. }, ; //X3_DECIMAL
	{ 'Rej.Ped.Compra'														, .T. }, ; //X3_TITULO
	{ 'Rej.Ped.Compra'														, .T. }, ; //X3_TITSPA
	{ 'Rej.Ped.Compra'														, .T. }, ; //X3_TITENG
	{ 'Rej.Ped.Compra'											, .T. }, ; //X3_DESCRIC
	{ 'Rej.Ped.Compra'											, .T. }, ; //X3_DESCSPA
	{ 'Rej.Ped.Compra'											, .T. }, ; //X3_DESCENG
	{ '@!'																	, .T. }, ; //X3_PICTURE
	{ ''																	, .T. }, ; //X3_VALID
	{ Chr(128) + Chr(128) + Chr(128) + Chr(128) + Chr(128) + ;
	Chr(128) + Chr(128) + Chr(128) + Chr(128) + Chr(128) + ;
...
aAdd( aSX6, { ;
	...
	'ES_GRPAP'																, ; //X6_VAR
	'C'																		, ; //X6_TIPO
	'Grupo Aprov Padrao'														, ; //X6_DESCRIC
	'Grupo Aprov Padrao'																		, ; //X6_DSCSPA
	'Grupo Aprov Padrao'																		, ; //X6_DSCENG
	'Grupo Aprov Padrao'																		, ; //X6_DESC1
	'Grupo Aprov Padrao'																		, ; //X6_DSCSPA1
	'Grupo Aprov Padrao'																		, ; //X6_DSCENG1
	''																		, ; //X6_DESC2
	''																		, ; //X6_DSCSPA2
	''																		, ; //X6_DSCENG2
	''																	, ; //X6_CONTEUD
...
aAdd( aSIX, { ;
	'SC1'																	, ; //INDICE
	'G'																		, ; //ORDEM
	'C1_FILIAL+C1_XAPROV'													, ; //CHAVE
	'Filial+Grp Aprov'														, ; //DESCRICAO
	'Filial+Grp Aprov'														, ; //DESCSPA
	'Filial+Grp Aprov'														, ; //DESCENG
	'U'																		, ; //PROPRI
	''																		, ; //F3
	'IND099SC1'																, ; //NICKNAME
	'S'																		} ) //SHOWPESQ
