...
/* ===============================================================================
WSDL Location    http://alliar.fluig.com/webdesk/ECMDatasetService?wsdl
Gerado em        04/18/19 13:39:26
Observações      Código-Fonte gerado por ADVPL WSDL Client 1.120703
                 Alterações neste arquivo podem causar funcionamento incorreto
                 e serão perdidas caso o código-fonte seja gerado novamente.
=============================================================================== */

User Function _ONUDXQR ; Return  // "dummy" function - Internal Use 
...
cSoap += WSSoapValue("name", ::cname, cname , "string", .T. , .T. , 0 , NIL, .F.,.F.) 
cSoap += "</q1:deleteDataset>"

oXmlRet := SvcSoapCall(	Self,cSoap,; 
	"deleteDataset",; 
	"RPCX","http://ws.dataservice.ecm.technology.totvs.com/",,,; 
	"http://alliar.fluig.com/webdesk/ECMDatasetService")
...
cSoap += WSSoapValue("companyId", ::ncompanyId, ncompanyId , "long", .T. , .T. , 0 , NIL, .F.,.F.) 
cSoap += WSSoapValue("username", ::cusername, cusername , "string", .T. , .T. , 0 , NIL, .F.,.F.) 
cSoap += WSSoapValue("password", ::cpassword, cpassword , "string", .T. , .T. , 0 , NIL, .F.,.F.) 
...
cSoap += WSSoapValue("password", ::cpassword, cpassword , "string", .T. , .T. , 0 , NIL, .F.,.F.) 
cSoap += WSSoapValue("name", ::cname, cname , "string", .T. , .T. , 0 , NIL, .F.,.F.) 
cSoap += WSSoapValue("description", ::cdescription, cdescription , "string", .T. , .T. , 0 , NIL, .F.,.F.) 
cSoap += WSSoapValue("impl", ::cimpl, cimpl , "string", .T. , .T. , 0 , NIL, .F.,.F.) 
cSoap += "</q1:addDataset>"
...
cSoap += WSSoapValue("password", ::cpassword, cpassword , "string", .T. , .T. , 0 , NIL, .F.,.F.) 
cSoap += WSSoapValue("name", ::cname, cname , "string", .T. , .T. , 0 , NIL, .F.,.F.) 
cSoap += WSSoapValue("fields", ::oWSgetDatasetfields, oWSgetDatasetfields , "stringArray", .T. , .T. , 0 , NIL, .F.,.F.) 
cSoap += WSSoapValue("constraints", ::oWSgetDatasetconstraints, oWSgetDatasetconstraints , "searchConstraintDtoArray", .T. , .T. , 0 , NIL, .F.,.F.) 
cSoap += WSSoapValue("order", ::oWSgetDatasetorder, oWSgetDatasetorder , "stringArray", .T. , .T. , 0 , NIL, .F.,.F.) 
cSoap += "</q1:getDataset>"
...
/*----------------------------------------
	11/04/2019 - Jonatas Oliveira - Compila
	######IMPORTANTE######
	
	Criado tratativa para atribuição de variável.
	Ao gerar o Client não está retornando os valores
	no objeto
...
WSMETHOD SOAPSEND WSCLIENT ECMDatasetServiceService_searchConstraintDtoArray
	Local cSoap := ""
	aEval( ::oWSitem , {|x| cSoap := cSoap  +  WSSoapValue("item", x , x , "searchConstraintDto", .F. , .T., 0 , NIL, .F.,.F.)  } ) 
Return cSoap
...
WSMETHOD SOAPSEND WSCLIENT ECMDatasetServiceService_searchConstraintDto
	Local cSoap := ""
	cSoap += WSSoapValue("contraintType", ::ccontraintType, ::ccontraintType , "string", .F. , .T., 0 , NIL, .F.,.F.) 
	cSoap += WSSoapValue("fieldName", ::cfieldName, ::cfieldName , "string", .F. , .T., 0 , NIL, .F.,.F.) 
	cSoap += WSSoapValue("finalValue", ::cfinalValue, ::cfinalValue , "string", .F. , .T., 0 , NIL, .F.,.F.) 
	cSoap += WSSoapValue("initialValue", ::cinitialValue, ::cinitialValue , "string", .F. , .T., 0 , NIL, .F.,.F.) 
	cSoap += WSSoapValue("likeSearch", ::llikeSearch, ::llikeSearch , "boolean", .F. , .T., 0 , NIL, .F.,.F.) 
Return cSoap
