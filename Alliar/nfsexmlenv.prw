...
user function nfseXMLEnv( cTipo, dDtEmiss, cSerie, cNota, cClieFor, cLoja, cMotCancela,aAIDF )
	...
	Local aRetIRR	:= {}
	Local aRetINS	:= {}
	Local cMsgAux, cMsgAlliar //| Allliar |  
	lOCAL cVenctos	:= "" //| Alliar |
	...
	If cTipo == "1" .And. Empty(cMotCancela)
		...
		If DbSeek(xFilial("SF2")+cNota+cSerie+cClieFor+cLoja)	
           ...
           If cMvMsgTrib $ "1-3"
			...
			If   (SED->ED_RECIRRF == "2") .or. (SED->ED_RECIRRF == "3"  .and. SA1->A1_RECIRRF  == "2")
				lRecIrrf:= .F.
			EndIf
			...
			While !(cAliasSD2)->(Eof()) .And. xFilial("SD2") == (cAliasSD2)->D2_FILIAL .And.;
				...
				IF SE1->(DBSEEK(SF2->F2_FILIAL+SF2->F2_SERIE+SF2->F2_DOC)) 
					
					cVenctos	:= ""
					WHILE SE1->(!EOF()) .AND.;
					 	(SE1->E1_FILIAL+SE1->E1_PREFIXO+SE1->E1_NUM) == (SF2->F2_FILIAL+SF2->F2_SERIE+SF2->F2_DOC)
					 	
					 	IF ALLTRIM(SE1->E1_TIPO) == "NF"
					 		IF !EMPTY(cVenctos)
					 			cVenctos := cVenctos+" / "
					 		ENDIF
					 		cVenctos	:= cVenctos+DTOC(SE1->E1_VENCREA)
					 	ENDIF
					...
					IF !EMPTY(cVenctos)
						...
						cMsgAlliar	+= "Vencimento em: "+cVenctos
					ENDIF
				...
				If !Empty(SC5->C5_MENPAD)
					cMsgAux := If(FindFunction('CleanSpecChar'),CleanSpecChar(AllTrim(FORMULA(SC5->C5_MENPAD))),AllTrim(FORMULA(SC5->C5_MENPAD)))
				EndIf
										
				
				If !Empty(cMsgAux)
					IF !EMPTY(cMsgAlliar)
						cMsgAlliar += CRLF+CRLF
					ENDIF
					cMsgAlliar += cMsgAux
				EndIf	
				
				/*------------------------------------------------------ Augusto Ribeiro | 27/12/2017 - 10:21:46 AM
					Lei da Transparencia
				------------------------------------------------------------------------------------------*/
				IF !("12.741/2012" $ cMsgAlliar)
					IF ALLTRIM(SM0->M0_CODMUN)=="3550308" //| São Paulo |
						cMsgAlliar += CRLF+"Lei Transparencia n 12.741/2012: o percentual aproximado dos tributos incidentes sobre os servicos e de 16,70%. (Fonte: IBPT Instituto Brasileiro de Planejamento Tributario)."
					ENDIF
				ENDIF
				...
				If dbSeek(xFilial("SX5")+"60"+RetFldProd(SB1->B1_COD,"B1_CODISS"))
					If !lNFeDesc
						If nCont == 1 
							cNatOper += If(FindFunction('CleanSpecChar'),CleanSpecChar(AllTrim(SubStr(SX5->(FIELDGET(FIELDPOS("X5_DESCRI"))),1,55))),AllTrim(SubStr(SX5->(FIELDGET(FIELDPOS("X5_DESCRI"))),1,55)))    			
	    				EndIf
		    		ElseIf nCont == 1 
							cDescrNFSe := If(FindFunction('CleanSpecChar'),CleanSpecChar(AllTrim(SubStr(SX5->(FIELDGET(FIELDPOS("X5_DESCRI"))),1,55))),AllTrim(SubStr(SX5->(FIELDGET(FIELDPOS("X5_DESCRI"))),1,55)))    			
		    		EndIf
    			EndIf
		...
		if !empty(aNota)
			if len (aProd) > 0
				...
				cString	+= servicos( aProd, aISSQN, aRetido, cNatOper, lNFeDesc, cDiscrNFSe, aCST, aDest[22], SM0->M0_CODMUN, cF4Agreg ,nDescon,lRecIrrf,lAglutina)
				cString	+= valores( aISSQN, aRetido, aTotal, aDest, SM0->M0_CODMUN,lRecIrrf )
...
/*/
//-----------------------------------------------------------------------
static function servicos( aProd, aISSQN, aRetido, cNatOper, lNFeDesc, cDiscrNFSe,aCST, cTpPessoa, cCodMun, cF4Agreg, nDescon, lRecIrrf,lAglutina)	
...
//			nValLiq	:= aProd[Nx][27] - aPisXml[1] - aCofinsXml[1] - aInssXml[1] - aIRRFXml[1] - aCSLLXml[1] - aIssRet[1]
		...
		Else
			cString	+= "<valunit>" + allTrim( convType( aProd[nX][28]/aProd[nX][09], 15, 2 ) ) + "</valunit>"
		EndIf
...
/*/
//-----------------------------------------------------------------------
static function valores( aISSQN, aRetido, aTotal, aDest, cCodMun, lRecIrrf )
